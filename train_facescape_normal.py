...
from models_normal import PointConvSceneFlowPWC8192selfglobalPointConv as PointConvSceneFlow
from models_normal import multiScaleLossAndSmoothLoss, multiScaleLoss
from pathlib import Path
from collections import defaultdict

from datasets.facescape_normal import FaceScapeDataSet
...
def main():
    ...
    os.system('cp %s %s' % ('train_facescape.py', log_dir))
    os.system('cp %s %s' % ('config_train_facescape.yaml', log_dir))
    ...
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[40, 46], gamma=0.1)
    ...
    for epoch in range(init_epoch, args.epochs):
        ...
        for i, data in tqdm(enumerate(train_loader, 0), total=len(train_loader), smoothing=0.9):
            pos1, pos2, color1, color2, normals1, normals2, flow, _ = data
            ...
            model = model.train()
            pred_flows, fps_pc1_idxs, _, _, pred_flow_ori = model(pos1, pos2, color1, color2, normals1, normals2)

            loss = multiScaleLoss(pred_flows, flow[:, :4096, :], fps_pc1_idxs)#loss = multiScaleLoss(pred_flows, flow_ori, pos1, flow, fps_pc1_idxs)
...
def eval_sceneflow(model, loader):
    ...
    for batch_id, data in tqdm(enumerate(loader), total=len(loader), smoothing=0.9):
        ...
        with torch.no_grad():
            ...
            #eval_loss = multiScaleLossAndSmoothLoss(pred_flows, pred_flow_ori, pos1, flow, fps_pc1_idxs)
            eval_loss = multiScaleLoss(pred_flows, flow[:, :4096, :], fps_pc1_idxs)
            # eval_loss = multiScaleLoss(pred_flows, flow, fps_pc1_idxs)

            epe3d = torch.norm(pred_flow_ori.permute(0, 2, 1) - flow, dim=2).mean()
