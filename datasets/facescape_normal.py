...
class FaceScapeDataSet(data.Dataset):
    ...
    def __getitem__(self, index):
        if index in self.cache:
            points1, points2, feat1, feat2, normals1, normals2, flow = self.cache[index]

        else:
            points1, points2, feat1, feat2, normal1, normal2 = self.data_loader(self.samples[index])
            flow = points2 - points1
            if len(self.cache) < self.cache_size:
                self.cache[index] = (points1, points2, feat1, feat2, normal1, normal2, flow)
        ...
        return points1, points2, feat1, feat2, normal1, normal2, flow, mask
    ...
    def data_loader(self, path):
        ...
        normal2 = data2['normals'].astype('float32')
        feature2 = np.hstack([color2 / 255., normal2])
        ...
        normal1 = data1['normals'].astype('float32')
        feature1 = np.hstack([color1 / 255., normal1])

        return points1, points2, feature1, feature2, normal1, normal2
