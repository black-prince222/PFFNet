...
def index_normals_group(normals, knn_idx):
    """
    Input:
        normals: input normals data, [B, N, 3]
        knn_idx: sample index data, [B, N, K]
    Return:
        new_normals:, indexed normals data, [B, N, K, 3]
    """
    normals_flipped = normals.permute(0, 2, 1).contiguous()
    new_normals = pointnet2_utils.grouping_operation(normals_flipped, knn_idx.int()).permute(0, 2, 3, 1)

    return new_normals



def normals_angle(cent_normal, nei_normal):
    """
    Input:
        cent_normal: central point's normal, [B, N, 3]
        nei_normal: neighbor points' normals, [B, N, k, 3]
    Output:
        angles: per-point normal angles, [B, N, k]
    """
    cent_normal = cent_normal.unsqueeze(2)
    angles = torch.sum(cent_normal * nei_normal, dim = 3) #[B,N,K]
    #denom = torch.linalg.norm(cent_normal, dim = 2) #* torch.linalg.norm(nei_normal, dim = 3)
...
def group_query(nsample, s_xyz, xyz, s_points, return_xyz=False):
    """
    Input:
        nsample: scalar
    ...
    if not return_xyz:
        return new_points, grouped_xyz_norm
    return new_points, grouped_xyz_norm, grouped_xyz
...
class AttentiveFlowPoP(nn.Module):
    def __init__(self, nsample, in_channel, mlp, bn=use_bn, use_leaky=True):
        super(AttentiveFlowPoP, self).__init__()
        ...
        for out_channel in mlp:
            ...
            if not use_leaky:
                self.mlp_convs.append(nn.ReLU(inplace=True))
            else:
                self.mlp_convs.append(nn.LeakyReLU(LEAKY_RATE, inplace=True))
            last_channel = out_channel
        self.mlp_convs = nn.Sequential(*self.mlp_convs)
        ...
        self.pos_mlp1 = nn.Sequential(
            ...
            nn.BatchNorm2d(16) if bn else nn.Identity(),
            nn.ReLU(inplace=True) if not use_leaky else nn.LeakyReLU(LEAKY_RATE, inplace=True),
            nn.Conv2d(16, last_channel, 1),
            nn.BatchNorm2d(last_channel) if bn else nn.Identity(),
            nn.ReLU(inplace=True) if not use_leaky else nn.LeakyReLU(LEAKY_RATE, inplace=True),
        )
        self.weightnet1 = WeightNet(last_channel * 2, last_channel, hidden_unit=[last_channel // 4, last_channel // 4])
    ...
    def forward(self, xyz1, xyz2, points1, points2):
        ...
        knn_idx1 = knn_point(self.nsample, xyz2, xyz1)  # B, N, nsample
        repeated_xyz = xyz1.view(B, N, 1, C).repeat(1, 1, self.nsample, 1)
        neighbor_xyz1 = index_points_group(xyz2, knn_idx1)
        dist_norm1 = torch.norm(neighbor_xyz1 - repeated_xyz, dim=-1, keepdim=True)  # [B, N, nsample, 1]
        direction_xyz1 = neighbor_xyz1 - repeated_xyz
        pos_info1 = torch.cat([repeated_xyz, neighbor_xyz1, direction_xyz1, dist_norm1], dim=-1)  # [B, N, nsample, 10]

        grouped_points2 = index_points_group(points2, knn_idx1)  # B, N, nsample, D
        repeated_points1 = points1.view(B, N, 1, D).repeat(1, 1, self.nsample, 1)
        new_points = torch.cat([repeated_points1, grouped_points2, pos_info1], dim=-1)  # B, N, nsample, D+D+10
        new_points = new_points.permute(0, 3, 2, 1)  # [B, D+D+10, nsample, N]
        new_points = self.mlp_convs(new_points)  # [B, D', nsample, N]

        # weighted sum
        pos_feat1 = self.pos_mlp1(pos_info1.permute(0, 3, 2, 1))  # [B, D', nsample, N]
        weights1 = self.weightnet1(torch.cat([pos_feat1, new_points], dim=1))  # B D' nsample N
        # weights1 = torch.softmax(weights1, dim=2)
        point_to_patch_cost = torch.sum(weights1 * new_points, dim=2)  # B D' N
        return point_to_patch_cost


class SeWeightNet(nn.Module):
    def __init__(self, in_channel, out_channel, hidden_unit=[8, 8], bn=use_bn):
        super(SeWeightNet, self).__init__()
        ...
        self.se = nn.Sequential(
            nn.Conv2d(out_channel, out_channel // 8, 1),
            nn.BatchNorm2d(out_channel // 8) if bn else nn.Identity(),
            nn.ReLU(inplace=True),
            nn.Conv2d(out_channel // 8, out_channel, 1),
            nn.Sigmoid())
    ...
    def forward(self, localized_xyz):
        ...
        for i, conv in enumerate(self.mlp_convs):
            if self.bn:
                ...
                weights = F.relu(conv(weights))
        se = self.se(torch.mean(weights, dim=2, keepdim=True))  # [B, C, 1, N]
        return weights * se
...
class PointConv(nn.Module):
    def __init__(self, nsample, in_channel, out_channel, weightnet=16, bn=use_bn, use_leaky=True):
        ...
        self.bn = bn
        self.nsample = nsample
        self.weightnet = SeWeightNet(3, weightnet)
...
class PointConvD(nn.Module):
    ...
    def forward(self, xyz, points, normals):
        ...
        fps_idx_normal = fps_idx.unsqueeze(2).repeat(1,1,3).to(torch.int64)
        new_normals = torch.gather(normals, dim=1, index=fps_idx_normal)
        ...
        return new_xyz.permute(0, 2, 1), new_points, fps_idx, new_normals


class PointConvAdaD(nn.Module):
    def __init__(self, npoint, nsample, in_channel, out_channel, weightnet=16, bn=use_bn, use_leaky=True):
        super(PointConvAdaD, self).__init__()
        self.npoint = npoint
        self.bn = bn
        self.nsample = nsample
        ...
        self.mlp = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv1d(in_ch // 4, in_ch // 8, 1),
            nn.ReLU(inplace=True),
            nn.Conv1d(in_ch // 8, 1, 1))
    ...
    def forward(self, xyz, points, normals):
        ...
        sample_score = self.mlp(points).squeeze(dim=1)  # [B, C]
        points = points.permute(0, 2, 1).contiguous()

        _, top_k_indices = torch.topk(sample_score, self.npoint, dim=-1, largest=False, sorted=False)
        fps_idx = top_k_indices.int()
        # fps_idx = pointnet2_utils.furthest_point_sample(xyz, self.npoint)
        new_xyz = index_points_gather(xyz, fps_idx)
...
class PointConvAdaDV1(nn.Module):
    def __init__(self, npoint, nsample, in_channel, out_channel, weightnet=16, bn=use_bn, use_leaky=True):
        super(PointConvAdaDV1, self).__init__()
        self.npoint = npoint
        self.bn = bn
        self.nsample = nsample
        self.weightnet = SeWeightNet(3, weightnet)
        ...
        self.pos_weightnet = WeightNet(in_channel, 1, hidden_unit=[in_channel // 4, in_channel // 4])
    ...
    def forward(self, xyz, points, normals):
        ...
        new_points, grouped_xyz_norm, xyz_group = group_query(self.nsample, xyz, new_xyz, points, return_xyz=True)

        pos_weights = self.pos_weightnet(new_points.permute(0, 3, 2, 1))
        pos_weights = torch.softmax(pos_weights, dim=2)  # [B, 1, K, N]
        xyz_group = xyz_group.permute(0, 3, 2, 1)  # [B, 3, K, N]
        new_xyz = torch.sum(pos_weights * xyz_group, dim=2)  # [B, 3, N]
...
class UpsampleFlow(nn.Module):
    def forward(self, xyz, sparse_xyz, normals, sparse_normals, sparse_flow, n=3, last =False):
        # import ipdb; ipdb.set_trace()
        ...
        #normals  B N 3
        #sparse_normals B S 3

        # knn_neigh = 7
        # knn_idx = knn_point(knn_neigh, sparse_xyz, xyz)  # [B, N, 5]
        # ##根据法向量筛除索引##
        # neign_normals = index_normals_group(sparse_normals, knn_idx)  # [B, N, 5, 3]
        # angles = normals_angle(normals, neign_normals)  # [B,N,k=5]
        # normals_idx = normal_point(n, angles, knn_idx)
        ...
        if last == True:
            ...
            knn_neigh = 3
            normals_idx = knn_point(knn_neigh, sparse_xyz, xyz)  # [B, N, 5]


        grouped_xyz_norm = index_points_group(sparse_xyz, normals_idx) - xyz.view(B, N, 1, C)
        ...
        grouped_flow = index_points_group(sparse_flow, normals_idx)
        dense_flow = torch.sum(weight.view(B, N, n, 1) * grouped_flow, dim=2).permute(0, 2, 1)
        return dense_flow


def normal_point(n, angles, knn_idx):
    """
    input:
        n: sorted point by normal angles
        angles: normal_angles [B,N,5]
        knn_idx: near neighbor index [B,N,5]
    output:
        normal_idx  [B,N,3]
    """
    ...
    #
    # B,N,K = knn_idx.shape
    # new_idx = torch.zeros((B,N,n), dtype=torch.int).cuda()
    # for i in range(N):
    #     mask = angles[:,i,:].ge(0)
    #     try:
    #         new_idx[:,i,:] = torch.masked_select(knn_idx[:,i,:], mask)[:n]
    #     except:
    #         #print(knn_idx[:,i,:])
    #         new_idx[:,i,:] = knn_idx[:,i,:][:,n]
    # return new_idx
    ...
    # mask = angles.ge(0)
    # print(mask)
    # knn_idx = torch.masked_select(knn_idx, mask).view(B,N,K)
    # print(knn_idx)

    #取n个法向量夹角最小的点
    _, idx = torch.topk(angles, k = n, dim = 2)
    normal_idx = torch.gather(knn_idx, dim = 2, index = idx)
    return normal_idx

class UpsampleFlowv1(nn.Module):
    def forward(self, xyz, sparse_xyz, normals, sparse_normals, sparse_flow, n=3, last = True):
        # import ipdb; ipdb.set_trace()
        ...
        grouped_xyz_norm = index_points_group(sparse_xyz, normals_idx) - xyz.view(B, N, 1, C)
        #grouped_xyz_norm = index_points_group(sparse_xyz, knn_idx) - xyz.view(B, N, 1, C)
        dist = torch.norm(grouped_xyz_norm, dim=3).clamp(min=1e-10) ** 2.
        norm = torch.sum(1.0 / dist, dim=2, keepdim=True)
        weight = (1.0 / dist) / norm
