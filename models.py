...
from pointconv_util import PointConv, PointConvD, PointWarping, UpsampleFlow, PointConvFlow, PointConvAdaDV1, \
    AttentiveFlowPoP
from pointconv_util import SceneFlowEstimatorPointConv, UpsampleFlowv1
...
class PointConvSceneFlowPWC8192selfglobalPointConv(nn.Module):
    def __init__(self):
        ...
        # l2: 512
        self.level2 = PointConvAdaDV1(512, feat_nei, 128 + 3, 128)
    ...
    def forward(self, xyz1, xyz2, color1, color2):
        ...
        pc2_l2_warp = self.warping(pc1_l2, pc2_l2, up_flow2) #src 向 tar移动
        ...
        feat2, flow2 = self.flow2(pc1_l2, new_feat1_l2, cost2, up_flow2) #得到当前的特征和场景流
        ...
        flows = [flow0, flow1, flow2, flow3]  #用于计算multiscale loss
        ...
        if xyz1_ori.shape[1] > 4096:
            if self.training:
                flow_ori = self.upsample(xyz1_ori.permute(0, 2, 1), xyz1.permute(0, 2, 1), flow0)
            else:
                flow_ori = self.upsamplev1(xyz1_ori.permute(0, 2, 1), xyz1.permute(0, 2, 1), flow0)
        else:
            flow_ori = flow0
