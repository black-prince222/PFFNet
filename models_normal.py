...
from pointconv_util_normal import PointConv, PointConvD, PointWarping, UpsampleFlow, PointConvFlow, PointConvAdaDV1, \
    AttentiveFlowPoP
from pointconv_util_normal import SceneFlowEstimatorPointConv, UpsampleFlowv1
from pointconv_util_normal import index_points_gather as index_points, index_points_group, Conv1d, square_distance
import time
...
class PointConvSceneFlowPWC8192selfglobalPointConv(nn.Module):
    def __init__(self):
        ...
        self.scale = scale
        # l0: 8192
        self.level0 = Conv1d(6, 32)
        self.level0_1 = Conv1d(32, 32)
        self.cost0 = AttentiveFlowPoP(flow_nei, 32 + 32 + 32 + 32 + 10, [32, 32])
        self.flow0 = SceneFlowEstimatorPointConv(32 + 64, 32)
        self.level0_2 = Conv1d(32, 64)

        # l1: 2048  下采样后的点
        self.level1 = PointConvD(2048, feat_nei, 64 + 3, 64)  #fps + Pconv，输出下采样后的点的坐标、特征、索引
        self.cost1 = AttentiveFlowPoP(flow_nei, 64 + 32 + 64 + 32 + 10, [64, 64])
        self.flow1 = SceneFlowEstimatorPointConv(64 + 64, 64)
        self.level1_0 = Conv1d(64, 64)
        self.level1_1 = Conv1d(64, 128)

        # l2: 512
        self.level2 = PointConvAdaDV1(512, feat_nei, 128 + 3, 128) #自适应下采样 + Pconv
        self.cost2 = AttentiveFlowPoP(flow_nei, 128 + 64 + 128 + 64 + 10, [128, 128])
        self.flow2 = SceneFlowEstimatorPointConv(128 + 64, 128)
        self.level2_0 = Conv1d(128, 128)
        self.level2_1 = Conv1d(128, 256)

        # l3: 256
        self.level3 = PointConvD(128, feat_nei, 256 + 3, 256)
        self.cost3 = AttentiveFlowPoP(flow_nei, 256 + 64 + 256 + 64 + 10, [256, 256])
        self.flow3 = SceneFlowEstimatorPointConv(256, 256, flow_ch=0)
        self.level3_0 = Conv1d(256, 256)
        self.level3_1 = Conv1d(256, 512)

        # l4: 64
        self.level4 = PointConvD(32, feat_nei, 512 + 3, 256)
        ...
        self.upsamplev1 = UpsampleFlowv1()

    def forward(self, xyz1, xyz2, feat1, feat2, normal1, normal2):
        xyz1_ori = xyz1
        normal1_ori = normal1
        if xyz1_ori.shape[1] > 4096:
            # xyz1 = xyz1[:, :4096, :]
            # xyz2 = xyz2[:, :4096, :]
            # feat1 = feat1[:, :4096, :]
            # feat2 = feat2[:, :4096, :]
            # normal1 = normal1[:, :4096, :]
            # normal2 = normal2[:, :4096, :]
            ####
            n1 = xyz1_ori.shape[1]
            sample_idx1 = np.random.choice(n1, 4096, replace=False)
            n2 = xyz1_ori.shape[1]
            sample_idx2 = np.random.choice(n2, 4096, replace=False)
            xyz1 = xyz1[:, sample_idx1, :]
            xyz2 = xyz2[:, sample_idx2, :]
            feat1 = feat1[:, sample_idx1, :]
            feat2 = feat2[:, sample_idx2, :]
            normal1 = normal1[:, sample_idx1, :]
            normal2 = normal2[:, sample_idx2, :]
            ####

        # xyz1, xyz2: B, N, 3
        # color1, color2: B, N, 3
        # normal1, normal2: B, N, 3

        # l0
        pc1_l0 = xyz1.permute(0, 2, 1)
        pc2_l0 = xyz2.permute(0, 2, 1)
        feat1 = feat1.permute(0, 2, 1)  # B 3 N
        feat2 = feat2.permute(0, 2, 1)  # B 3 N

        feat1_l0 = self.level0(feat1)
        feat1_l0 = self.level0_1(feat1_l0)
        feat1_l0_1 = self.level0_2(feat1_l0)

        feat2_l0 = self.level0(feat2)
        feat2_l0 = self.level0_1(feat2_l0)
        feat2_l0_1 = self.level0_2(feat2_l0)

        # l1
        pc1_l1, feat1_l1, fps_pc1_l1, normal1_l1 = self.level1(pc1_l0, feat1_l0_1, normal1)
        feat1_l1_2 = self.level1_0(feat1_l1)
        feat1_l1_2 = self.level1_1(feat1_l1_2)

        pc2_l1, feat2_l1, fps_pc2_l1, normal2_l1 = self.level1(pc2_l0, feat2_l0_1, normal2)
        feat2_l1_2 = self.level1_0(feat2_l1)
        feat2_l1_2 = self.level1_1(feat2_l1_2)

        # l2
        pc1_l2, feat1_l2, fps_pc1_l2, normal1_l2 = self.level2(pc1_l1, feat1_l1_2, normal1_l1)
        feat1_l2_3 = self.level2_0(feat1_l2)
        feat1_l2_3 = self.level2_1(feat1_l2_3)

        pc2_l2, feat2_l2, fps_pc2_l2, normal2_l2 = self.level2(pc2_l1, feat2_l1_2, normal2_l1)
        feat2_l2_3 = self.level2_0(feat2_l2)
        feat2_l2_3 = self.level2_1(feat2_l2_3)

        # l3
        pc1_l3, feat1_l3, fps_pc1_l3, normal1_l3 = self.level3(pc1_l2, feat1_l2_3, normal1_l2)
        feat1_l3_4 = self.level3_0(feat1_l3)
        feat1_l3_4 = self.level3_1(feat1_l3_4)

        pc2_l3, feat2_l3, fps_pc2_l3, normal2_l3 = self.level3(pc2_l2, feat2_l2_3, normal2_l2)
        feat2_l3_4 = self.level3_0(feat2_l3)
        feat2_l3_4 = self.level3_1(feat2_l3_4)

        # l4
        pc1_l4, feat1_l4, _, normal1_l4 = self.level4(pc1_l3, feat1_l3_4, normal1_l3)
        feat1_l4_3 = self.upsample(pc1_l3, pc1_l4, normal1_l3, normal1_l4, feat1_l4)
        feat1_l4_3 = self.deconv4_3(feat1_l4_3)

        pc2_l4, feat2_l4, _, normal2_l4 = self.level4(pc2_l3, feat2_l3_4, normal2_l3)
        feat2_l4_3 = self.upsample(pc2_l3, pc2_l4, normal2_l3, normal2_l4, feat2_l4)
        ...
        feat1_l3_2 = self.upsample(pc1_l2, pc1_l3, normal1_l2, normal1_l3, feat1_l3)
        feat1_l3_2 = self.deconv3_2(feat1_l3_2)

        feat2_l3_2 = self.upsample(pc2_l2, pc2_l3, normal2_l2, normal2_l3, feat2_l3)
        ...
        feat1_l2_1 = self.upsample(pc1_l1, pc1_l2, normal1_l1, normal1_l2, feat1_l2)
        feat1_l2_1 = self.deconv2_1(feat1_l2_1)

        feat2_l2_1 = self.upsample(pc2_l1, pc2_l2, normal2_l1, normal2_l2, feat2_l2)
        ...
        feat1_l1_0 = self.upsample(pc1_l0, pc1_l1, normal1, normal1_l1, feat1_l1)
        feat1_l1_0 = self.deconv1_0(feat1_l1_0)

        feat2_l1_0 = self.upsample(pc2_l0, pc2_l1, normal2, normal2_l1, feat2_l1)
        ...
        # if xyz1_ori.shape[1] > 4096:
        #     flow_ori = self.upsamplev1(xyz1_ori.permute(0, 2, 1), xyz1.permute(0, 2, 1), flow0)
        # else:
        #     flow_ori = flow0
        if xyz1_ori.shape[1] > 4096:
            if self.training:
                flow_ori = self.upsample(xyz1_ori.permute(0, 2, 1), xyz1.permute(0, 2, 1), normal1_ori, normal1, flow0)
            else:
                flow_ori = self.upsamplev1(xyz1_ori.permute(0, 2, 1), xyz1.permute(0, 2, 1), normal1_ori, normal1, flow0)
        else:
            flow_ori = flow0

        return flows, fps_pc1_idxs, fps_pc2_idxs, xyz1, flow_ori


def multiScaleLossAndSmoothLoss(pred_flows, flow_ori, pc1, gt_flow, fps_idxs, alpha=[0.02, 0.04, 0.08, 0.16]):
    # num of scale
    num_scale = len(pred_flows)
    offset = len(fps_idxs) - num_scale + 1

    # generate GT list and mask1s
    gt_flows = [gt_flow[:, :pred_flows[0].shape[-1], :]]
    ...
    # pred_flow = flow_ori.permute(0, 2, 1)
    ...
    # smooth_loss = torch.mean(torch.sum(diff_flow, dim=1))
